...
class VideoBatcherSync(GstBase.Aggregator):
    __gstmetadata__ = ('video_batcher_sync','Video/Mixer', \
...
# required fields:
    ...
    def __init__(self):
        ...
        try:
            super(VideoBatcherSync, self).__init__()
            self.external_iter = []
        ...
        try:
            ...
            for entry in self.table:
                ...
                for size in sizes:
                    ...
                    #with open('/frames/img-{}.jpeg'.format(key), 'wb') as writer:
                    #    writer.write(encoded_data[frame_idx])
            ...
            outbuf.fill(0, self.d_to_send)
            outbuf.pts = self.current_pts
...
GObject.type_register(VideoBatcherSync)
__gstelementfactory__ = ("video_batcher_sync", Gst.Rank.NONE, VideoBatcherSync)
