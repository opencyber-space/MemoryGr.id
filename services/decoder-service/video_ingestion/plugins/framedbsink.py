...
class FramedbRouter :
  ...
  def __init__(self):
    ...
    source_data = json.loads(os.getenv('SOURCE_DATA', {}))

    print(source_data)

    router_info = source_data['router']

    ROUTING_URI  = router_info['routing_uri'] if 'routing_uri' not in router_info else 'http://localhost:8000'

    ROUTING_URI = router_info['routing_api'] if 'routing_api' in router_info else '/routing/getMapping'

    UPDATE_API = router_info['update_api'] if 'update_api' in router_info else '/routing/updateMapping'

    ROUTING_PUBSUB_HOST = os.getenv("PUB_SUB_SVC", "localhost")
    #ROUTING_PUBSUB_PORT = int(os.getenv("PUBSUB_PORT", 6379))
    ROUTING_PUBSUB_PASSWORD = os.getenv("PUB_SUB_PASSWORD", "Friends123#")
    REDIS_CLUSTER_MODE = int(os.getenv("REDIS_CLUSTER_MODE", "0"))
    ROUTING_PUBSUB_PORT = 6379 if not  REDIS_CLUSTER_MODE else 26379
    ...
    self.cameraId = source_data['source_id']
